...
### 3.1 SLO 카탈로그 (실측 기반 갱신 2026-05-28 — `prd-phase4/results/final_2026-05-28.md` §4.2; SLO-1 / SLO-4 는 `prd-phase4/03` §4.5 변형으로 잠정값 유지)
...
| **SLO-2 (등록 지연)** | `POST /webhooks`의 응답 지연 p99가 7.5ms 이내 | `histogram_quantile(0.99, sum by (le) (rate(webhook_relay_api_request_duration_seconds_bucket{route="/webhooks"}[5m])))` | **p99 ≤ 7.5ms** | 7일 |
| **SLO-3 (전달 지연)** | 작업 등록 → 외부 송신 성공까지의 wall-clock p99가 14.9ms 이내 (재시도 미포함, 첫 시도 기준) | `histogram_quantile(0.99, sum by (le) (rate(webhook_relay_worker_processing_duration_seconds_bucket{outcome="success"}[5m])))` | **p99 ≤ 14.9ms** | 7일 |
...
### 3.4 SLO 임계 — 실측 기반 갱신 (2026-05-28, `docs/prd-phase4/results/final_2026-05-28.md` §4.2)

위 목표값은 **4단계 실측 분포에 기반한 갱신값** 이다. 잠정 상태였던 임계 (99.5%
/ 0.5s / 5s / 1%) 는 본 PRD 본문이 정의한 SLI PromQL / 측정 윈도우 / error
budget 공식의 형태 잠금은 유지한 채로 임계 숫자만 갱신되었다 (I6.1 정합).

- 본 PRD 가 정의한 **형태** (SLI PromQL, 윈도우 정의, error budget 공식, burn
  rate 14.4× / 6× 표준값) 는 **변경 0건** (I6.1 / I6.2 정합).
- 본 PRD 가 정의한 **목표 숫자** 는 4단계 PRD `prd-phase4/03` §4 의 재조정
  규칙 (Q-LOAD-9 (a) p99 × 1.5) 으로 갱신:
  - SLO-2 등록 지연: `p99 ≤ 0.5s` → `p99 ≤ 7.5ms` (실측 nominal 영역 max
    4.99ms × 1.5 = 7.485ms 반올림).
  - SLO-3 전달 지연: `p99 ≤ 5s` → `p99 ≤ 14.9ms` (실측 nominal 영역 max
    9.96ms × 1.5 = 14.94ms 반올림).
- **SLO-1 (가용성) / SLO-4 (DLQ 적재율) 의 잠정값 유지**: 4단계 PRD
  `prd-phase4/03` §4.5 변형 적용 — 비율 SLI 의 실측 평균 × 1.5 규칙은 실측이
  0 인 경우 잠정값 유지. nominal 영역 (LP-1 / LP-2 normal / LP-4 base /
  M-LOAD-5 4 N) 의 실측 5xx 비율 0% + DLQ 적재율 0% 가 본 변형 발동.
- 갱신된 임계로 알람 발화 정확도 변동을 후속 측정으로 확인 (별도 commit
  시리즈, `prd-phase4/03` §6.4 정합).
...
| SLO-2 등록 지연 (7d) | p99 ≤ 7.5ms | 7d × 1% = **약 1.68시간** p99 초과 허용 |
| SLO-3 전달 지연 (7d) | p99 ≤ 14.9ms | 7d × 1% = **약 1.68시간** p99 초과 허용 |
...
### 5.3 잠금된 항목 vs 실측 기반 갱신 vs 결정 보류
...
본 PRD 의 임계 숫자 — **4단계 실측 기반 갱신** (Q-OBS-11 closed, 2026-05-28
`docs/prd-phase4/results/final_2026-05-28.md` §4.2 정합):

- SLO-1 가용성 5xx 비율 ≤ 0.5% (실측 0 → `prd-phase4/03` §4.5 변형으로 잠정값
  유지)
- SLO-2 등록 지연 p99 ≤ 7.5ms (실측 nominal 영역 max 4.99ms × 1.5)
- SLO-3 전달 지연 p99 ≤ 14.9ms (실측 nominal 영역 max 9.96ms × 1.5)
- SLO-4 DLQ 적재율 ≤ 1% (실측 0 → `prd-phase4/03` §4.5 변형으로 잠정값 유지)

본 PRD가 **결정 보류** (`05` §2):

- burn rate를 14.4×/6×에서 다르게 잠글 것인가 (Q-OBS-12)
