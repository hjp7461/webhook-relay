...
export function createConnection(opts: ConnectionOptions): Redis {
  const base = Math.max(1, opts.reconnectBaseMs);
  const cap = Math.max(base, opts.reconnectMaxMs);
  const ioOpts: RedisOptions = {
    // BullMQ 권장 — blocking 명령(brpoplpush 등) 사용 시 retry-per-request 제거.
    maxRetriesPerRequest: null,
    enableReadyCheck: false,
    retryStrategy(times: number): number {
      // 지수 백오프: base * 2^(times-1), 상한 cap.
      const delay = Math.min(cap, base * 2 ** Math.max(0, times - 1));
      return delay;
    },
