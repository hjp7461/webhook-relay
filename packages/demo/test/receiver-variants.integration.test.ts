...
//   5) variant=s3    → HMAC 헤더 값별 K=2 회 503 후 200 (IT-S3 부하 변형)
//   6) variant=s3 + HMAC 헤더 부재 → 503 reason='no_hmac' (방어적 가드)
//   7) variant=s3 + 다른 HMAC → 새 카운터 슬롯, 1차 503  (카운터 키 격리)
...
describe("/_demo/receiver variant-aware stub responses", () => {
  ...
  it("variant=s3 + HMAC 헤더 부재 → 503 + reason='no_hmac'", async () => {
    const res = await fetch(`${app.baseUrl}/_demo/receiver?variant=s3`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ no: "hmac" }),
    });
    expect(res.status).toBe(503);
    const body = (await res.json()) as Record<string, unknown>;
    expect(body).toMatchObject({ ok: false, variant: "s3", reason: "no_hmac" });
  });
});
