...
> **단일 출처는 git/PLAN/코드**. HANDOFF.md 는 보조 컨텍스트일 뿐이다. 불일치 시 git/PLAN 을 신뢰하고 사용자에게 알린 뒤 진행한다(`/checkpoint` 로 갱신 권장).
...
- **HEAD SHA 가 다름** → HANDOFF.md 가 stale. 새 세션이지만 사용자가 직전에 `/checkpoint` 없이 다른 작업을 했을 가능성. **사용자에게 알리고** "`/checkpoint` 로 갱신하시겠습니까?" 제안. 그동안은 git log/PLAN 본문을 1차 출처로 사용.
...
세션 종료 직전(또는 사용자가 `/clear` 를 예고할 때) **`/checkpoint`** 을 권장. 본 명령은 `.claude/state/HANDOFF.md` 를 갱신 + commit + push 해 다음 세션이 정확한 위치에서 재개할 수 있게 한다.

> `/checkpoint` 가 호출되지 않으면 HANDOFF.md 는 직전 스냅샷에 머무른다 — `/resume` 의 §3 재검증이 차이를 잡지만, 차이가 클수록 더 많은 추론이 필요해 비용이 증가한다.
